    ...
    def isNStraightHand(self, hand, groupSize) -> bool:
        ...
        while minH:
            ...
            for i in range(first, first + groupSize):
                if i not in count:
                    return False
                count[i] -=1 #zmniejszamy ilosc wystapien!
                if count[i] == 0: #jesli zdazy sie ze mamy zero wystapien
                    if i != minH[0]: #i okaze sie ze nie jest na poczatku
                        return False 
                    heapq.heappop(minH) #usuwamy tę wartosc
        return True        