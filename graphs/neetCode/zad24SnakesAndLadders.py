...
class Solution:
    def snakesAndLadders(self, board: list[list[int]]) -> int:
        length = len(board)
        board.reverse() # poniewaz startujemy od dolu i idziemy do gory wezem, musimy zrewersowac cala tablice!
        def intToPos(square):
            r = (square - 1) // length
            c = (square -1) % length
            if r % 2: c = length -1 -c
        ...
        q = deque() # [square,moves] moves - how many moves we did do get to this square (sth like distance)
        q.append([1,0])
        visit =  set()
        while q:
            ...
            for i in range(1,7):
                ...
                if board[r][c] != -1:
                    nextSquare = board[r][c] #natrafiliysmy na weza lub drabine! wiec przenosimy sie w tą wartość!
                if nextSquare == length**2: return moves + 1
